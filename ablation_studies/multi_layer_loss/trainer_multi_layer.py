...
from ablation_studies.multi_layer_loss.models.torch_model import EfficientAdModel, EfficientAdModelSize, reduce_tensor_elems
...
class EfficientADTrainer:
    ...
    def compute_teacher_mean_std(self, dataloader: DataLoader) -> dict[str, torch.Tensor]:
        """Calculate channel-wise mean and std of teacher model outputs for conv6.

        Args:
            dataloader (DataLoader): Training dataloader.

        Returns:
            dict[str, torch.Tensor]: Dictionary with conv6 'mean' and 'std' tensors
        """
        self.model.eval()
        n_conv6 = None
        channel_sum_conv6 = None
        channel_sum_sqr_conv6 = None

        for batch in tqdm.tqdm(dataloader, desc="Computing teacher statistics (conv6)"):
            images = batch['image'].to(self.device) if isinstance(batch, dict) else batch[0].to(self.device)

            _, _, conv6_output = self.model._forward_with_intermediate(self.model.teacher, images)

            if n_conv6 is None:
                _, num_channels_conv6, _, _ = conv6_output.shape
                n_conv6 = torch.zeros((num_channels_conv6,), dtype=torch.int64, device=conv6_output.device)
                channel_sum_conv6 = torch.zeros((num_channels_conv6,), dtype=torch.float32, device=conv6_output.device)
                channel_sum_sqr_conv6 = torch.zeros((num_channels_conv6,), dtype=torch.float32, device=conv6_output.device)

            n_conv6 += conv6_output[:, 0].numel()
            channel_sum_conv6 += torch.sum(conv6_output, dim=[0, 2, 3])
            channel_sum_sqr_conv6 += torch.sum(conv6_output**2, dim=[0, 2, 3])

        channel_mean_conv6 = channel_sum_conv6 / n_conv6
        channel_std_conv6 = torch.sqrt((channel_sum_sqr_conv6 / n_conv6) - (channel_mean_conv6**2))

        return {
            "mean": channel_mean_conv6.float()[None, :, None, None],
            "std": channel_std_conv6.float()[None, :, None, None],
        }
    ...
    def train_step(self, batch) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            tuple: (loss_conv6, loss_conv5, loss_conv4, loss_ae, loss_stae, loss_st_total) - loss components.
        """
        self.model.train()
        ...
        # Forward pass
        loss_conv6, loss_conv5, loss_conv4, loss_ae, loss_stae, loss_st_total = self.model(batch=images, batch_imagenet=batch_imagenet)

        # Backward pass
        total_loss = loss_st_total + loss_ae + loss_stae
        self.optimizer.zero_grad()
        total_loss.backward()
        self.optimizer.step()
        self.scheduler.step()

        return loss_conv6, loss_conv5, loss_conv4, loss_ae, loss_stae, loss_st_total
    ...
    def fit(
        ...
        # Compute teacher statistics for conv6
        if not self.model.is_set(self.model.mean_std):
            logger.info("Computing teacher channel statistics for conv6...")
            mean_std_conv6 = self.compute_teacher_mean_std(train_dataloader)
            self.model.mean_std.update(mean_std_conv6)
            logger.info("✓ Conv6 statistics computed and stored")
        ...
        for epoch in range(max_epochs):
            # Training
            epoch_losses = {"conv6": [], "conv5": [], "conv4": [], "ae": [], "stae": [], "st_total": []}

            pbar = tqdm.tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{max_epochs}")
            for batch in pbar:
                loss_conv6, loss_conv5, loss_conv4, loss_ae, loss_stae, loss_st_total = self.train_step(batch)

                epoch_losses["conv6"].append(loss_conv6.item())
                epoch_losses["conv5"].append(loss_conv5.item())
                epoch_losses["conv4"].append(loss_conv4.item())
                epoch_losses["ae"].append(loss_ae.item())
                epoch_losses["stae"].append(loss_stae.item())
                epoch_losses["st_total"].append(loss_st_total.item())

                # Update progress bar
                pbar.set_postfix({
                    "conv6": f"{loss_conv6.item():.4f}",
                    "conv5": f"{loss_conv5.item():.4f}",
                    "conv4": f"{loss_conv4.item():.4f}",
                    "ae": f"{loss_ae.item():.4f}",
                    "stae": f"{loss_stae.item():.4f}",
                })

            # Log epoch statistics
            avg_loss_conv6 = sum(epoch_losses["conv6"]) / len(epoch_losses["conv6"])
            avg_loss_conv5 = sum(epoch_losses["conv5"]) / len(epoch_losses["conv5"])
            avg_loss_conv4 = sum(epoch_losses["conv4"]) / len(epoch_losses["conv4"])
            avg_loss_ae = sum(epoch_losses["ae"]) / len(epoch_losses["ae"])
            avg_loss_stae = sum(epoch_losses["stae"]) / len(epoch_losses["stae"])
            avg_loss_st_total = sum(epoch_losses["st_total"]) / len(epoch_losses["st_total"])

            logger.info(
                f"Epoch {epoch+1} - "
                f"conv6: {avg_loss_conv6:.4f}, "
                f"conv5: {avg_loss_conv5:.4f}, "
                f"conv4: {avg_loss_conv4:.4f}, "
                f"st_total: {avg_loss_st_total:.4f}, "
                f"ae: {avg_loss_ae:.4f}, "
                f"stae: {avg_loss_stae:.4f}"
            )
