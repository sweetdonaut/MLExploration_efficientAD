...
from efficientad.data import InferenceBatch
...
class EfficientAdModelSize(str, Enum):
    ...
    Example:
        ...
        >>> model_size = EfficientAdModelSize.DINO
        >>> model_size
        <EfficientAdModelSize.DINO: 'dinov3'>
    """
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size: int = 16, in_chans: int = 3, embed_dim: int = 384) -> None:
        super().__init__()
...
class DINOv3PatchDescriptionNetwork(nn.Module):
    """DINOv3 ViT-Small as teacher network.

    Uses a pretrained DINOv3 Vision Transformer (ViT-Small) with patch size 16
    to extract spatial features from images.

    Args:
        out_channels (int): Number of output channels (should be 384 for ViT-Small).
        padding (bool, optional): Ignored for compatibility. Defaults to False.

    Note:
        Input images should be 896x896 to produce 56x56 spatial output.
        Output shape: (batch, 384, 56, 56)
    """
    ...
    def __init__(self, out_channels: int, padding: bool = False) -> None:
        ...
        self.reg_token = nn.Parameter(torch.zeros(1, 4, self.embed_dim))

        self.blocks = nn.ModuleList([
            TransformerBlock(self.embed_dim, self.num_heads) for _ in range(self.depth)
        ])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (N, 3, 896, 896).

        Returns:
            torch.Tensor: Output feature maps of shape (N, 384, 56, 56).
        """
        x = imagenet_norm_batch(x)
...
class EfficientAdModel(nn.Module):
    ...
    def __init__(
        ...
        self.teacher: MediumPatchDescriptionNetwork | SmallPatchDescriptionNetwork | DINOv3PatchDescriptionNetwork
        ...
        elif model_size == EfficientAdModelSize.DINO:
            self.teacher = DINOv3PatchDescriptionNetwork(out_channels=teacher_out_channels, padding=padding).eval()
            self.student = MediumPatchDescriptionNetwork(out_channels=teacher_out_channels * 2, padding=padding)
    ...
    def forward(
        ...
        """
        student_output, distance_st_conv6, teacher_conv4, student_conv4, teacher_conv5, student_conv5 = self.compute_student_teacher_distance(batch)
        if self.training:
            return self.compute_losses(batch, batch_imagenet, distance_st_conv6, teacher_conv4, student_conv4, teacher_conv5, student_conv5)

        map_st, map_stae = self.compute_maps(batch, student_output, distance_st_conv6, normalize)
        anomaly_map = 0.5 * map_st + 0.5 * map_stae
        pred_score = torch.amax(anomaly_map, dim=(-2, -1))
        return InferenceBatch(pred_score=pred_score, anomaly_map=anomaly_map)

    def _forward_with_intermediate(self, model, x):
        """Forward pass that returns conv4, conv5, conv6 outputs.

        Returns:
            tuple: (x_conv4_before_relu, x_conv5_before_relu, x_conv6)
                - x_conv4_before_relu: Conv4 output BEFORE ReLU (has negative values)
                - x_conv5_before_relu: Conv5 output BEFORE ReLU (has negative values)
                - x_conv6: Conv6 output (after Conv5 ReLU)
        """
        x = imagenet_norm_batch(x)
        ...
        # Get Conv4 output BEFORE ReLU
        x_conv4_before_relu = model.conv4(x)
        x_conv4_after_relu = F.relu(x_conv4_before_relu)

        # Get Conv5 output BEFORE ReLU
        x_conv5_before_relu = model.conv5(x_conv4_after_relu)

        # Apply ReLU and continue to Conv6
        x_conv5_after_relu = F.relu(x_conv5_before_relu)
        x_conv6 = model.conv6(x_conv5_after_relu)

        return x_conv4_before_relu, x_conv5_before_relu, x_conv6

    def compute_student_teacher_distance(self, batch: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Compute the student-teacher distance vectors for conv4, conv5, and conv6.
        ...
        Returns:
            tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
                - Student network output features (conv6)
                - Squared distance between normalized teacher and student features (conv6)
                - Teacher conv4 features BEFORE ReLU (has negative values)
                - Student conv4 features BEFORE ReLU (has negative values)
                - Teacher conv5 features BEFORE ReLU (has negative values)
                - Student conv5 features BEFORE ReLU (has negative values)
        """
        with torch.no_grad():
            teacher_conv4, teacher_conv5, teacher_conv6 = self._forward_with_intermediate(self.teacher, batch)

            if self.is_set(self.mean_std):
                teacher_conv6 = (teacher_conv6 - self.mean_std["mean"]) / self.mean_std["std"]

        if batch.shape[-2:] != (256, 256):
            batch_student = F.interpolate(batch, size=(256, 256), mode="bilinear", align_corners=False)
        else:
            batch_student = batch

        student_conv4, student_conv5, student_conv6 = self._forward_with_intermediate(self.student, batch_student)

        distance_st_conv6 = torch.pow(teacher_conv6 - student_conv6[:, : self.teacher_out_channels, :, :], 2)

        return student_conv6, distance_st_conv6, teacher_conv4, student_conv4, teacher_conv5, student_conv5

    def compute_losses(
        self,
        batch: torch.Tensor,
        batch_imagenet: torch.Tensor,
        distance_st_conv6: torch.Tensor,
        teacher_conv4: torch.Tensor,
        student_conv4: torch.Tensor,
        teacher_conv5: torch.Tensor,
        student_conv5: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Compute training losses.

        Computes loss components:
        - Conv6 loss (L2 distance with hard example mining)
        - Conv5 loss (L2 distance with hard example mining, before ReLU)
        - Conv4 loss (L2 distance with hard example mining, before ReLU)
        ...
        Args:
            ...
            distance_st_conv6 (torch.Tensor): Student-teacher distances for conv6.
            teacher_conv4 (torch.Tensor): Teacher conv4 features.
            student_conv4 (torch.Tensor): Student conv4 features.
            teacher_conv5 (torch.Tensor): Teacher conv5 features.
            student_conv5 (torch.Tensor): Student conv5 features.
        ...
        Returns:
            tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
                ...
                - Total student-teacher loss
        """
        # Conv6 loss (L2 distance with hard example mining)
        distance_st_conv6_flat = reduce_tensor_elems(distance_st_conv6)
        d_hard_conv6 = torch.quantile(distance_st_conv6_flat, 0.999)
        loss_hard_conv6 = torch.mean(distance_st_conv6_flat[distance_st_conv6_flat >= d_hard_conv6])

        # Conv5 loss (L2 distance, computed on features BEFORE ReLU)
        student_conv5_trimmed = student_conv5[:, : self.teacher_out_channels, :, :]
        distance_st_conv5 = torch.pow(teacher_conv5 - student_conv5_trimmed, 2)
        distance_st_conv5_flat = reduce_tensor_elems(distance_st_conv5)
        d_hard_conv5 = torch.quantile(distance_st_conv5_flat, 0.999)
        loss_hard_conv5 = torch.mean(distance_st_conv5_flat[distance_st_conv5_flat >= d_hard_conv5])

        # Conv4 loss (L2 distance, computed on features BEFORE ReLU)
        # Conv4 has different channel count than Conv5/Conv6, so match teacher's channel count
        teacher_conv4_channels = teacher_conv4.shape[1]
        student_conv4_trimmed = student_conv4[:, :teacher_conv4_channels, :, :]
        distance_st_conv4 = torch.pow(teacher_conv4 - student_conv4_trimmed, 2)
        distance_st_conv4_flat = reduce_tensor_elems(distance_st_conv4)
        d_hard_conv4 = torch.quantile(distance_st_conv4_flat, 0.999)
        loss_hard_conv4 = torch.mean(distance_st_conv4_flat[distance_st_conv4_flat >= d_hard_conv4])

        if batch_imagenet.shape[-2:] != (256, 256):
            batch_imagenet_student = F.interpolate(batch_imagenet, size=(256, 256), mode="bilinear", align_corners=False)
        else:
            batch_imagenet_student = batch_imagenet
        student_output_penalty = self.student(batch_imagenet_student)[:, : self.teacher_out_channels, :, :]
        loss_penalty = torch.mean(student_output_penalty**2)

        loss_st_total = (1.0/3.0) * loss_hard_conv6 + (1.0/3.0) * loss_hard_conv5 + (1.0/3.0) * loss_hard_conv4 + loss_penalty

        # Autoencoder and Student AE Loss
        aug_img = self.choose_random_aug_image(batch)
        if aug_img.shape[-2:] != (256, 256):
            aug_img_student = F.interpolate(aug_img, size=(256, 256), mode="bilinear", align_corners=False)
        else:
            aug_img_student = aug_img
        ...
        ae_output_aug = self.ae(aug_img_student, (256, 256))
        student_output_ae_aug = self.student(aug_img_student)[:, self.teacher_out_channels :, :, :]
        ...
        return (loss_hard_conv6, loss_hard_conv5, loss_hard_conv4, loss_ae, loss_stae, loss_st_total)
    ...
    def compute_maps(
        ...
        with torch.no_grad():
            if batch.shape[-2:] != (256, 256):
                batch_ae = F.interpolate(batch, size=(256, 256), mode="bilinear", align_corners=False)
            else:
                batch_ae = batch
            ae_output = self.ae(batch_ae, (256, 256))
    ...
    def get_maps(self, batch: torch.Tensor, normalize: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        student_output, distance_st_conv6, teacher_conv4, student_conv4, teacher_conv5, student_conv5 = self.compute_student_teacher_distance(batch)
        return self.compute_maps(batch, student_output, distance_st_conv6, normalize)
