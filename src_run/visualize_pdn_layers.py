...
from efficientad.models.torch_model import EfficientAdModel, EfficientAdModelSize, imagenet_norm_batch
...
class PDNLayerExtractor:
    ...
    def compute_all_layers_mean_std(self, dataloader):
        ...
        layer_names = [
            ...
            'after_conv3_relu', 'after_conv4_relu',
            'after_conv5_relu', 'after_conv6'
        ]
        ...
        with torch.no_grad():
            for images, _ in tqdm(dataloader, desc="Computing statistics"):
                ...
                x_t = F.relu(teacher.conv4(x_t))
                self._update_stats(layer_stats['after_conv4_relu'], x_t)
                x_t = F.relu(teacher.conv5(x_t))
                self._update_stats(layer_stats['after_conv5_relu'], x_t)
    ...
    def extract_all_layers(self, x):
        ...
        with torch.no_grad():
            ...
            x_t = F.relu(teacher.conv4(x_t))
            self.activations['teacher_after_conv4_relu'] = x_t.detach()
            x_t = F.relu(teacher.conv5(x_t))
            self.activations['teacher_after_conv5_relu'] = x_t.detach()
            x_t = teacher.conv6(x_t)
        ...
        x_s = F.relu(student.conv4(x_s))
        self.activations['student_after_conv4_relu'] = x_s.detach()
        x_s = F.relu(student.conv5(x_s))
        self.activations['student_after_conv5_relu'] = x_s.detach()
...
def compute_layer_heatmap(student_layer, teacher_layer, layer_name, layer_mean_std, image_size=(256, 256)):
    if layer_name in layer_mean_std:
        ...
        valid_channels = (std.squeeze() > 1e-6)

        if valid_channels.any():
            teacher_layer = teacher_layer[:, valid_channels, :, :]
            mean = mean[:, valid_channels, :, :]
            std = std[:, valid_channels, :, :]
...
def create_layer_visualization(image_path, layer_heatmaps, layer_names, vis_size=(256, 256)):
    ...
    rf_sizes = {
        ...
        'after_conv4_relu': '21x21',
        'after_conv5_relu': '33x33',
        'after_conv6': '33x33'
    }
...
output_dir = Path("results_standalone/EfficientAd") / dataset_name / f"{category}{model_size_suffix}" / "pdn_layers/test"
